...
export function useAssinaturaPage() {
      ...
      if (matched) {
        ...
        if (
          ...
          const trialCycle: BillingCycle =
            cycleFromUrl === 'yearly' || cycleFromUrl === 'monthly' ? cycleFromUrl : selectedCycle
  ...
  useEffect(() => {
    ...
    if (isActive || hasPaidPayment) {
      ...
      refreshToken()
        .catch(() => {
          console.warn('Token refresh failed after subscription confirmation')
        })
  ...
  const handleRegisterAndContinue = async (
    ...
    try {
      ...
      toast.error(Array.isArray(msg) ? msg[0] : msg || 'Erro ao criar conta. Tente novamente.')
    } finally {
  ...
  const completedTrialDays = completedReason === 'trial' ? (selectedPlan?.trial_days ?? null) : null
